...
defmodule HL7.Message.Test do
  ...
  test "Read/write complete trimmed request in wire format" do
    ...
    "PR1|2||904620^^99DH\r" <>
    "AUT||112233||||||1|0\r"
    assert trim === read_and_write(orig, format: :wire, trim: true)
  end
  ...
  test "Read/write complete trimmed request in stdio format" do
    ...
    AUT||112233||||||1|0
    """
    assert trim === read_and_write(orig, format: :stdio, trim: true)
  end
  ...
  test "Read trimmed request in wire format and write it untrimmed in stdio format" do
    ...
    AUT||112233^^^^^||||||1|0|
    """
    {:ok, msg} = HL7.read(trim, format: :wire, trim: true)
    gen = HL7.write(msg, format: :stdio, trim: false)
  ...
  test "Read partial response in wire format" do
    ...
    "ZAU|||B004^PRESTACION AUTORIZADA|||0&$^\r" <>
    "NTE|1|||\r"
    {:incomplete, {function, rest}} = HL7.read(part1, format: :wire, trim: true)
    rest = rest <> part2
  ...
  test "Retrieve segments from message" do
    ...
    "ZAU|||B004^PRESTACION AUTORIZADA|||0&$^\r" <>
    "NTE|1|||\r"
    reader = HL7.Reader.new(format: :wire, trim: true)
  ...
  defp read_and_write(orig, options) do
    reader = HL7.Reader.new(options)
    {:ok, msg} = HL7.Message.read(reader, orig)
    writer = HL7.Writer.new(options)
    gen = HL7.Message.write(writer, msg)
    IO.iodata_to_binary(gen)
  end
